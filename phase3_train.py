...
def action_callback(action_raw):
    if action_raw[1] > 1:
        action_raw[1] = 1.0
    leagal_action = [-1, 0, 1]
    action = {"radar_angle": leagal_action[int(action_raw[2])],  # normalize(action_raw[0], [0, 1], [-45, 45]),
...
def train():
    ...
    actor_path = "./models/phase3/trained_actor.pkl"
    critic_path = "./models/phase3/trained_critic.pkl"
    actor_encoder_path = "./models/phase3/5trained_actor_encoder.pkl"
    radar_path = "./models/phase3/1detector_actor.pkl"
    agent = fak_model.AgentPhase3(device, actor_path, critic_path, actor_encoder_path, radar_path)
    ...
    with torch.autograd.set_detect_anomaly(True):
        ...
        engine = NaiveEngine([learner], data_manager, env_class=Fak, env_num=3,max_episode_length=1000,
                             obs_callback=obs_callback,
                             should_stop_callback=should_stop_callback,
                             action_callback=action_callback)
...
def render_show(_freq_f, _freq_d, _model_path: dict):
    ...
    agent = fak_model.AgentPhase3(device,
                                  _model_path["actor_path"],
                                  _model_path["critic_path"],
                                  _model_path["actor_encoder_path"],
                                  _model_path["radar_path"])
    agent.load(_model_path['total_path'])
    env.render()
    for i in range(10):
...
def validation(freq_f, freq_d, _model_path: dict):
    # validation
    # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    device = torch.device('cpu')
    agent = fak_model.AgentPhase3(device,
    ...
    with Manager() as manager:
        ...
        df.to_csv(f'./analysis/MRL_f{freq_f}_d{freq_d}.csv')
