...
from env import Fak
from airctrl.algorithm.ppo.learner import Learner
from airctrl.algorithm.ppo.memory import Memory
...
from baseline.PPO.model import Agent as PPO
...
def train():
    ...
    optim = Adam(filter(lambda x: x.requires_grad, list(agent.actor.parameters()) + list(agent.critic.parameters())), lr=1e-3)
    learner = Learner(agent, 512, optim=optim)
...
def render_show(_freq_f, _freq_d, model_path="./models/best/1"):
...
def validation(freq_f, freq_d, model_path="./models/best/"):
    ...
    with Manager() as manager:
        ...
        df.to_csv(f'../../analysis/PPO/PPO_f{freq_f}_d{freq_d}.csv')
...
if __name__ == '__main__':
    ...
    # # train()
    # # render_show(freq_fire, freq_detect,"./models/eval/")
    # for freq_fire, freq_detect in zip(freq_fire_list, freq_detect_list):
    #     print(freq_fire, freq_detect)
    #     validation(freq_fire, freq_detect, "./models/eval/")
    validation(0, 0, "./models/eval/")
